...
extern void nbody_serial(int numSrcs, float sx[], float sy[], float sz[], float ss[],
                         int numTarg, float tx[], float ty[], float tz[], float ts[], float tm[],
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < numTargs; i++) {
        ...
        ts[i] = 2.*(float)rand()/(float)RAND_MAX - 1.0;
        tm[i] = (float)rand()/(float)RAND_MAX + 1.e-6;
    }
    ...
    for (unsigned int i = 0; i < test_iterations[0]; ++i) {
        ...
        nbody_ispc(numSrcs, sx, sy, sz, ss, numTargs, tx, ty, tz, ts, tm, tax, tay, taz);
    ...
    for (unsigned int i = 0; i < test_iterations[1]; ++i) {
        reset_and_start_timer();
        nbody_serial(numSrcs, sx, sy, sz, ss, numTargs, tx, ty, tz, ts, tm, tax, tay, taz);
        double dt = get_elapsed_mcycles();
