...
extern void nbody_serial(int numSrcs, float sx[], float sy[], float ss[],
...
int main(int argc, char *argv[]) {
    ...
    for (unsigned int i = 0; i < test_iterations[0]; ++i) {
        ...
        nbody_ispc(numSrcs, sx, sy, ss, numTargs, tx, ty, ts, tax, tay);
    ...
    for (int i = 0; i < 2; i++) printf("   particle %d accel %g %g\n",i,tax[i],tay[i]);
    ...
    for (unsigned int i = 0; i < test_iterations[1]; ++i) {
        reset_and_start_timer();
        nbody_serial(numSrcs, sx, sy, ss, numTargs, tx, ty, ts, tax, tay);
        double dt = get_elapsed_mcycles();
